...
echo 
url=$1
echo "* = May take too long to convert!"
...
select option in $options; do
	...
	if [ $res = "best" ]; then
		yt-dlp -f "bestvideo+bestaudio" -o "/data/data/com.termux/files/home/storage/shared/Youtube/%(title)s-%(id)s.%(ext)s" --recode-video mp4 $url 
	elif [ $res = "audio" ]; then
		yt-dlp -x --audio-format mp3 --audio-quality 0 -o "/data/data/com.termux/files/home/storage/shared/Youtube/%(title)s.%(ext)s" $url
	else
		yt-dlp -f "bestvideo[height=?$res][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=?$res][ext=webm]+bestaudio[ext=m4a]/bestvideo+bestaudio[ext=m4a]" -o "/data/data/com.termux/files/home/storage/shared/Youtube/%(title)s-%(id)s.%(ext)s" --recode-video mp4 $url
	fi
