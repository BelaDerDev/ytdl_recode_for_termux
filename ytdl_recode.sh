...
select option in $options; do
	...
	if [ $res = "best" ]; then
		...
		yt-dlp -f "bestvideo[height=?$res][ext=mp4]/bestvideo[height<=?$res][ext=webm]/bestvideo+bestaudio" -o "/data/data/com.termux/files/home/storage/shared/Youtube/%(title)s-%(id)s.%(ext)s" --recode-video mp4 $url
	fi
